...
|------|------|--------|------|------|----------|
| INV-01 | `[PLANNED]` | 检查流程不得阻断、取消或延迟用户的提交动作；检查的全部失败都必须以「放行」收尾 | `core/riskEngine` | 内部 | 本文 §1 + `00` §3 可用性 |
| INV-02 | `[PLANNED]` | 任何离开本机的 payload 只含 diff hunk 与该 hunk 的限长上下文（单块 ≤ 2KB），**不得包含完整源文件** | `core/contextBuilder` | 出站 | 本文 §2.1 Request |
| INV-03 | `[PLANNED]` | Jev API Key 不得以明文出现在工作区文件、日志、遥测、错误信息或 UI 文本中 | `infra/secrets` | 内部 | 本文 §3 |
| INV-04 | `[PLANNED]` | Jev 不可用（网络失败 / 超时 / 配额耗尽 / 非 2xx）时必须降级为「静默放行 + 一次性提示」，不得向上抛错终止流程 | `infra/jevClient` | 内部 | 本文 §4 `ERR-01`–`ERR-04` |
| INV-05 | `[PLANNED]` | 检查过程对工作区严格只读：不得修改、暂存、格式化或删除任何文件 | `infra/git` | 内部 | `02` §3 边界说明 |
| INV-06 | `[PLANNED]` | 清单中每一项必须可定位到真实存在的文件与行号（禁止展示无法跳转的条目） | `ui/riskList` | 内部 | 本文 §2.2 |
| INV-07 | `[PLANNED]` | 风险阈值等判定参数必须来自命名常量或配置，禁止在判定逻辑中硬编码字面量 | `core/threshold` | 内部 | `01` §2 依赖限制 |
...
- 状态：`[PLANNED]`
- 不变性：任意出站请求体中，单块内容长度 ≤ 2048 字节，且不含 hunk 之外的未改动文件全文。
```
...
- **生效版本**：`v0.1.0` 的判定器只产出标 `v0.1.0` 的 5 项（`CONFIG_CHANGE` / `HIGH_FANOUT` 需配置解析与引用计数能力，推迟至 `v0.1.1`，见 `200-spec.md` §1.1）。本表为**完整枚举集合**，未生效项仍属合规取值，仅当前实现不会产出。
...
| 错误码 | 状态 | 含义 | 级别 | 处理建议 | 生效版本 |
|--------|------|------|------|----------|----------|
| `ERR-01` | `[PLANNED]` | 网络不可达 | WARN | 降级放行 + 一次性提示（INV-04） | `v0.1.1` |
| `ERR-02` | `[PLANNED]` | 请求超时 | WARN | 降级放行 + 一次性提示（INV-04） | `v0.1.1` |
| `ERR-03` | `[PLANNED]` | 非 2xx 响应 | WARN | 降级放行，日志记录状态码 | `v0.1.1` |
| `ERR-04` | `[PLANNED]` | 配额耗尽 / 鉴权失败 | ERROR | 降级放行 + 引导检查 API Key | `v0.1.1` |
| `ERR-05` | `[PLANNED]` | 响应 schema 不合规 | ERROR | 丢弃该块，写入日志，不进入清单 | `v0.1.1` |
| `ERR-06` | `[PLANNED]` | 非 git 仓库 | WARN | 提示一次并终止 | `v0.1.0` |
| `ERR-07` | `[PLANNED]` | 暂存区无改动 | INFO | 提示一次「无暂存改动」 | `v0.1.0` |
| `ERR-08` | `[PLANNED]` | API Key 未配置 | ERROR | 提示一次并给出 `sonecheck.setApiKey` 指引 | `v0.1.1` |
| `ERR-09` | `[PLANNED]` | git 可执行文件缺失 | ERROR | 提示一次并终止 | `v0.1.0` |
| `ERR-10` | `[PLANNED]` | 用户取消输入 | INFO | 忽略，保持原值 | `v0.1.1` |
| `ERR-11` | `[PLANNED]` | 输入为空串（清除 Key） | WARN | 二次确认后清除 | `v0.1.1` |

- **生效版本**：`v0.1.0` 只落地 `ERR-06` / `ERR-07` / `ERR-09`（本地失败路径，不依赖网络与密钥）；`ERR-01`~`ERR-05` 随真实判定服务、`ERR-08` / `ERR-10` / `ERR-11` 随 `API-03` 命令在 `v0.1.1` 落地（见 `05` §2.1 / §2.2）。本表为**完整错误码集合**，未生效项仍属合规返回值，仅当前实现不会产出。
