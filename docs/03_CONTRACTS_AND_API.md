...
- **超时**：单请求超时上限 <!-- TODO: [dm-init-docs] S3 实测回填 -->，且必须落在单块判定预算内（`00` §3：≤ 500ms）；超时视为 `ERR-02` 走降级
...
- **重试策略**：仅对 `429` / `529` 做指数退避（上限 <!-- TODO: [dm-init-docs] S3 实测回填 -->），其余状态码不重试；含退避在内的总耗时不得超过端到端预算（`00` §3：p95 ≤ 1s）。本项目**手写 `fetch` + 退避函数**，不引入官方 SDK（避免额外依赖，见 `01` §2 依赖限制）。
