...
| 阶段 | 目标 | 范围 | 完成判据 | 状态 |
|------|------|------|----------|------|
| M0 名称阵地 | 抢占免费标识，避免后期改名 | npm `sonecheck@0.0.1` 占位包（见 `01` §4）；GitHub 仓库 `sonecheck`（先 Private） | `npm view sonecheck` 可查到 0.0.1 | ⬜ |
...
|-------|------|------|------|----------|------|
| A（单点验证） | `v0.0.1` | MVP 主线：抓暂存区 diff → 切块 → Jev 判定 → Top-K 清单 → 跳转 | M0 完成 | 预估 S4/S5 各一次；一句话可描述「提交前挑出最该看的几块」 | ⬜ |
| A（单点验证） | `v0.1.0` | 判定质量：上下文增强 + LSP 元数据 + 阈值配置 | `v0.0.1` 真机可用 | 预估 S4/S5 各一次；一句话「让判定更准、可调」 | ⬜ |
...
- **npm 包与插件版本解耦**：`sonecheck` npm 包承载未来 CLI 能力，其版本号**不跟随** VS Code 插件版本；两者各自独立演进（`01` §4）。
...
| 强制拦截合规 | v0.0.1 不做强制拦截，避免误报导致用户绕过工具（`--no-verify` 式排斥） | `00` §5 | ⬜ |
