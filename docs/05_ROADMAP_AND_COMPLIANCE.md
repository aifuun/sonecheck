...
| M1 本地 MVP | 单机跑通「抓 diff → 判定 → 跳转」 | `02` §3 主链路（步骤 1–9），判定服务先用 mock，无 UI 美化 | 真机一次 ≥10 块改动内出清单并成功跳转 | `v0.1.0` | ⬜ |
...
| # | 成果 |
|---|------|
| 1 | 上下文增强：作用域行数可配置，命中率与上下文长度可权衡 |
| 2 | LSP 元数据接入：`caller_count` 改用真实引用数替代启发式估算 |
| 3 | 阈值与清单条数配置项暴露：`riskThreshold` / `maxItems` 可由用户设定 |
| 4 | 误报反馈入口：记录「命中但用户判定为误报」的条目 |
| 5 | 判定质量指标：命中率与误报计数可查看 |
...
> 本版范围在 Epoch A 实测结论出来前不细化（`<!-- TODO -->`），避免凭空预设。

---

### 2.5 版本规则补充说明

- **本项目编号变通**：规范 §3.3 使用号段式编号（`v20`–`v29` 为一个 Epoch），本项目改用 **semver** —— **主版本号即 Epoch 分段**（Epoch A = `0.x`；Epoch B = `1.x`），「群内连续、群间留白」的目的一致。版本目录名遵循 `docs/versions/<版本号>-<slug>/`，如 `v0.1.0-local-mvp`。
...
| 决策服务接入参数（endpoint / 鉴权方式）待确认 | 真实客户端无法开工 | `v0.1.0` 用 mock 判定器跑通链路，接口层隔离；`v0.1.1` 接入真实服务时只替换 `infra/jevClient` 实现 | 项目作者 |
...
| `v0.1.0` 设想的阈值与实际不符 | 判定质量差或清单过噪 | 强制走 Step S3：由 S2 Harness 实测数据回填阈值，禁止凭空预设（规范 §6.3） | 项目作者 |
