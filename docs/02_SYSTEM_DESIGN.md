...
| 关键路径 | 是否高开销 | 须含指标 | 落点 |
|----------|-----------|----------|------|
| git diff 读取（`infra/git`） | 是（子进程） | Elapsed / 改动块数 / 是否为空 | `06_OBSERVABILITY.md` |
| Jev 判定（`infra/jevClient`） | 是（网络） | Elapsed / 请求块数 / 状态码 / 重试次数 | `06_OBSERVABILITY.md` |
| 阈值过滤与 Top-K（`core/threshold`） | 否 | 输入块数 / 命中块数 / 阈值取值 | `06_OBSERVABILITY.md` |
| 降级路径（`Degraded`） | 否 | 失败原因码 / 是否静默 | `06_OBSERVABILITY.md` |
| 跳转定位（`ui/riskList`） | 否 | 目标文件 / 行列 / 是否命中 | `06_OBSERVABILITY.md` |
